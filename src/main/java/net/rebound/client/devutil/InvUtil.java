...
public class InvUtil {
    ...
    public static boolean testInMainHand(Predicate<ItemStack> predicate) {
        return predicate.test(Rebound.mc.player.getMainHandStack());
    }
    ...
    public static boolean testInOffHand(Predicate<ItemStack> predicate) {
        return predicate.test(Rebound.mc.player.getOffHandStack());
    }
    ...
    public static boolean testInHotbar(Predicate<ItemStack> predicate) {
        ...
        for (int i = SlotUtil.HOTBAR_START; i < SlotUtil.HOTBAR_END; i++) {
            ItemStack stack = Rebound.mc.player.getInventory().getStack(i);
    ...
    public static FindItemResult findInHotbar(Predicate<ItemStack> isGood) {
        if (testInOffHand(isGood)) {
            return new FindItemResult(SlotUtil.OFFHAND, Rebound.mc.player.getOffHandStack().getCount());
        }

        if (testInMainHand(isGood)) {
            return new FindItemResult(Rebound.mc.player.getInventory().selectedSlot, Rebound.mc.player.getMainHandStack().getCount());
        }
    ...
    public static FindItemResult find(Predicate<ItemStack> isGood) {
        if (Rebound.mc.player == null) return new FindItemResult(0, 0);
        return find(isGood, 0, Rebound.mc.player.getInventory().size());
    }
    ...
    public static void dropHand() {
        if (!Rebound.mc.player.currentScreenHandler.getCursorStack().isEmpty()) Rebound.mc.interactionManager.clickSlot(Rebound.mc.player.currentScreenHandler.syncId, ScreenHandler.EMPTY_SPACE_SLOT_INDEX, 0, SlotActionType.PICKUP, Rebound.mc.player);
    }
    ...
    public static class Action {
        ...
        private void run() {
            boolean hadEmptyCursor = Rebound.mc.player.currentScreenHandler.getCursorStack().isEmpty();
            ...
            if (!isRecursive && hadEmptyCursor && preType == SlotActionType.PICKUP && preTwo && (preFrom != -1 && preTo != -1) && !Rebound.mc.player.currentScreenHandler.getCursorStack().isEmpty()) {
                isRecursive = true;
        ...
        private void click(int id) {
            Rebound.mc.interactionManager.clickSlot(Rebound.mc.player.currentScreenHandler.syncId, id, data, type, Rebound.mc.player);
        }
    }
}
