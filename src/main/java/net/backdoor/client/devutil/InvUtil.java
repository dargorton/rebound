...
public class InvUtil {
    ...
    private void InvUtils() {
    }

    // Predicates

    public static boolean testInMainHand(Predicate<ItemStack> predicate) {
        return predicate.test(Backdoor.mc.player.getMainHandStack());
    }
    ...
    public static boolean testInOffHand(Predicate<ItemStack> predicate) {
        return predicate.test(Backdoor.mc.player.getOffHandStack());
    }
    ...
    public static boolean testInHotbar(Predicate<ItemStack> predicate) {
        ...
        for (int i = SlotUtil.HOTBAR_START; i < SlotUtil.HOTBAR_END; i++) {
            ItemStack stack = Backdoor.mc.player.getInventory().getStack(i);
    ...
    public static FindItemResult findInHotbar(Predicate<ItemStack> isGood) {
        if (testInOffHand(isGood)) {
            return new FindItemResult(SlotUtil.OFFHAND, Backdoor.mc.player.getOffHandStack().getCount());
        }

        if (testInMainHand(isGood)) {
            return new FindItemResult(Backdoor.mc.player.getInventory().selectedSlot, Backdoor.mc.player.getMainHandStack().getCount());
        }
    ...
    public static FindItemResult find(Predicate<ItemStack> isGood) {
        if (Backdoor.mc.player == null) return new FindItemResult(0, 0);
        return find(isGood, 0, Backdoor.mc.player.getInventory().size());
    }
    ...
    /*public static boolean swap(int slot, boolean swapBack) {
        ...
        if (swapBack && previousSlot == -1) previousSlot = Sapporo.mc.player.getInventory().selectedSlot;
        else if (!swapBack) previousSlot = -1;

        Sapporo.mc.player.getInventory().selectedSlot = slot;
        ((IClientPlayerInteractionManager) Sapporo.mc.interactionManager).meteor$syncSelected();
        return true;
    }
    ...
    public static void dropHand() {
        if (!Backdoor.mc.player.currentScreenHandler.getCursorStack().isEmpty()) Backdoor.mc.interactionManager.clickSlot(Backdoor.mc.player.currentScreenHandler.syncId, ScreenHandler.EMPTY_SPACE_SLOT_INDEX, 0, SlotActionType.PICKUP, Backdoor.mc.player);
    }
    ...
    public static class Action {
        ...
        private void run() {
            boolean hadEmptyCursor = Backdoor.mc.player.currentScreenHandler.getCursorStack().isEmpty();
            ...
            if (!isRecursive && hadEmptyCursor && preType == SlotActionType.PICKUP && preTwo && (preFrom != -1 && preTo != -1) && !Backdoor.mc.player.currentScreenHandler.getCursorStack().isEmpty()) {
                isRecursive = true;
        ...
        private void click(int id) {
            Backdoor.mc.interactionManager.clickSlot(Backdoor.mc.player.currentScreenHandler.syncId, id, data, type, Backdoor.mc.player);
        }
    }
}
