...
public class SlotUtil {
    ...
    private void SlotUtils() {
    }

    public static int indexToId(int i) {
        if (Backdoor.mc.player == null) return -1;
        ScreenHandler handler = Backdoor.mc.player.currentScreenHandler;
    ...
    /*private static int creativeInventory(int i) {
        if (!(Sapporo.mc.currentScreen instanceof CreativeInventoryScreen) || CreativeInventoryScreenAccessor.getSelectedTab() != Registries.ITEM_GROUP.get(ItemGroups.INVENTORY))
            return -1;
